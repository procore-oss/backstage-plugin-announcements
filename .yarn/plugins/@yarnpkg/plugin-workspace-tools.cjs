    ...
    var lt = ct(),
      pe = (e, t, r) => {
        ...
        let A = ft(e) || ft(t),
          ...
          y = [];
        if (
          (A && ((p.isPadded = A), (p.maxLen = String(p.max).length)), g < 0)
        ) {
...
`,
  ...
  var Pt = K((hs, Nt) => {
    ...
    var en = ke(),
      ...
      hn = (e, t = {}) => {
        ...
        let r = t || {},
          n = typeof r.maxLength == 'number' ? Math.min(It, r.maxLength) : It;
        if (e.length > n)
      ...
      nr = (e, t) => {
        ...
        let H = [],
          ...
          v = m => {
            if (c.type === 'globstar') {
              let L =
                  l.braces > 0 && (m.type === 'comma' || m.type === 'brace'),
                d =
                  m.extglob === !0 ||
            ...
            p === !1
              ...
              : h(f)
                ? (typeof s.onIgnore == 'function' && s.onIgnore(R),
                  (R.isMatch = !1),
                  A ? R : !1)
                : (typeof s.onMatch == 'function' && s.onMatch(R), A ? R : !0)
          );
        };
  ...
  var hr = K((_s, ur) => {
    ...
    N.capture = (e, t, r) => {
      let n = Ve.isWindows(r),
        a = oe
          .makeRe(String(e), Q(B({}, r), { capture: !0 }))
          .exec(n ? Ve.toPosixSlashes(t) : t);
    ...
    N.parse = (e, t) => {
      ...
      for (let n of [].concat(e || []))
        for (let s of pr(String(n), t)) r.push(oe.parse(s, t));
      return r;
    };
    N.braces = (e, t) => {
      if (typeof e != 'string') throw new TypeError('Expected a string');
  ...
  var Ie = X(require('@yarnpkg/cli')),
    ...
    xe = class extends Ie.BaseCommand {
      constructor() {
        ...
        this.jobs = U.Option.String('-j,--jobs', {
          ...
          validator: re.isOneOf([
            re.isEnum(['unlimited']),
            re.applyCascade(re.isNumber(), [re.isInteger(), re.isAtLeast(1)]),
          ]),
        });
      ...
      async execute() {
        ...
        if (!this.all && !n)
          throw new Ie.WorkspaceRequiredError(r.cwd, this.context.cwd);
        ...
        for (let E of f)
          (a &&
            !E.manifest.scripts.has(a) &&
            !p &&
            !(await ge.scriptUtils.getWorkspaceAccessibleBinaries(E)).has(a)) ||
          ...
          ne = await Ee.StreamReport.start(
            ...
            async E => {
                ...
                try {
                  ...
                  if (this.verbose) {
                    let ee = t.get('enableTimers')
                      ? `, completed in ${Y.formatUtils.pretty(t, $ - u, Y.formatUtils.Type.DURATION)}`
                      : '';
                    E.reportInfo(
                      null,
              ...
              for (; x.size > 0 && !E.hasErrors(); ) {
                ...
                for (let [H, w] of x) {
                  ...
                  if (this.topological || this.topologicalDev) {
                    ...
                    for (let u of c.values()) {
                      ...
                      if (
                        ((j =
                          I === null || !x.has(I.anchoredLocator.locatorHash)),
                        !j)
                      )
                        break;
                    }
                ...
                if (C.length === 0) {
                  let H = Array.from(x.values())
                    .map(w => Y.structUtils.prettyLocator(t, w.anchoredLocator))
                    .join(', ');
                  E.reportError(
  ...
  function _r(e, { prefix: t, interlaced: r }) {
    let n = e.createStreamReporter(t),
      s = new Y.miscUtils.DefaultStream();
    s.pipe(n, { end: !1 }),
      s.on('finish', () => {
        n.end();
      });
